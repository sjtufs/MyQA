...
if((isset($_GET['id']))&&(is_numeric($_GET['id'])))
{
	...
	echo '<tr><td align="left"><font size="10">' . $row['votes'] . '</font></td>
		<td align="left">
		<a href="handle_q_vote_detail.php?id=' . $qid . '">+</a>
	...
	require_once('./mysql_connect.php');
	$query="SELECT a_votes as votes, DATE_FORMAT(answertime,'%M %d, %Y') as dr,answer_content as content,answerer as answerer,answer_id as id  From answers WHERE father_id=$qid";
	$result = @mysql_query ($query);	
	...
	if($num>0)
		...
		while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
			...
			<td align="left">
			<a href="handle_a_vote.php?id=' . $qid . '">+</a>
			<input type="hidden" name="submitted" value="TRUE" />
			</td>
			<td align="left">' . $row['content'] . '</td>
...
<p class="pos_abs"><textarea name="answer" rows="20" cols="80"></textarea></p>
<input class="pos_abc" type ="submit" value="Answer it"/> 
</form> 
';
